...
public class GridWindow : EditorWindow
{
		...
		private string[] tile_CollisionModels = new string[]
	{
		...
		private int maximumRowCount = 10; //I highly suggest you dont use more than 10 rows because of internal use of the code flow.
		...
		private bool grid_SpawnRestriction = true;
		
		private Vector3 mousePositionGUI;
		private Vector3 handleRectOffset;
		private Vector3 handleRectOffsetZero;
		...
		{
				handleRectOffset = new Vector3 (grid_WidthHeight.x / 2f, grid_WidthHeight.y / 2f, 0f);
				handleRectOffsetZero = Vector3.zero;
		}
		...
		private void SaveEditorWindowSettings ()
		{
				...
				for (int i = 0; i < rowDataList.Count; i++) {
						rowAndTagList.Add (rowDataList [i].RowName);
						for (int k = 0; k < rowDataList [i].TagDataList.Count; k++) {
								rowAndTagList.Add (i + rowDataList [i].TagDataList [k].TagName);
						}
				}
		
				for (int m = 0; m < rowAndTagList.Count; m++) {
						rowTagData += rowAndTagList [m].ToString ();
						if (m < rowAndTagList.Count - 1) {
								rowTagData += ",";
						}
				}
		
				myTextWriter.Write (rowTagData);   
				myTextWriter.Close ();
		...
		{
				StreamReader myStreamReader = new StreamReader (savedRowTagDataLocation);
				string tagDataString = myStreamReader.ReadToEnd ();
				myStreamReader.Close ();
		
				if (System.String.IsNullOrEmpty (tagDataString)) {
						return;
				}
		
				string[] tags = tagDataString.Split (',');
				...
				for (int i = 0; i < tags.Length; i++) {   
						...
						if (!match.Success) {
								...
								rowData.RowName = tags [i].ToString ();
								...
								for (int j = 0; j < rowDataList.Count; j++) {
										if (int.Parse (match.Value) == j) {
												...
												tagData.TagName = tag;
												rowDataList [j].TagDataList.Add (tagData);
										}
								}
						}
				}
		...
		private bool SetPalletteBrushOption ()
		{
		...
		private void DrawTheExtractedSpritesInTheEditorWindow ()
		{
				for (int i = 0; i < drawTextureInWindowList.Count; i++) {
						...
						int row = (int)(Mathf.Floor (i / (maxTilesPerRowEditorWindow != 0 ? maxTilesPerRowEditorWindow : maxTilesPerRowEditorWindow + 1)) * yPos_Offset); 
						int collumn = (int)((maxTilesPerRowEditorWindow != 0 ? maxTilesPerRowEditorWindow : maxTilesPerRowEditorWindow + 1) * row);
						...
						if (GUI.Button (new Rect (xPos_StartPos + xPos_Offset * i - collumn, yPos_StartPos + row, previewButtonWH, previewButtonWH), drawTextureInWindowList [i])) {
								...
								CreateTileBrushActive = SetPalletteBrushOption ();
						}
				}
		}
		...
		private void UpdateEditorWindowInterface ()
		{
				...
				if (GUILayout.Button ("Add Row" + " (" + currentRowCount.ToString () + "/" + maximumRowCount.ToString () + ")", EditorStyles.miniButtonLeft, GUILayout.Width (position.width * 0.14f), GUILayout.Height (buttonHeight))) {
						if (rowDataList.Count >= maximumRowCount) { //Cant add more rows if cap of 10 has been reached.
								return;
						}
				...
				if (s_rowName.Length >= 1 && char.IsDigit (s_rowName [0])) {
						s_rowName = Regex.Replace (s_rowName, @"[^a-zA-Z]", string.Empty);   //First character of the string cannot be a number.
				}   
				...
				for (int i = 0; i < rowDataList.Count; i++) {
						if (rowDataList [i].RowActive = GUILayout.Toggle (rowDataList [i].RowActive, rowDataList [i].RowName, EditorStyles.miniButtonLeft, (rowDataList [i].RowActive ? GUILayout.Width (position.width * 0.885f) : GUILayout.Width (position.width)), GUILayout.Height (buttonHeight))) {
								...
								if (s_tagName.Length >= 1 && char.IsDigit (s_tagName [0])) {
										s_tagName = Regex.Replace (s_rowName, @"[^a-zA-Z]", string.Empty);   //First character of the string cannot be a number.
								}
				...
				GUILayout.Label (new GUIContent ("Tile name: " + tile_Name.ToString (), "The name of the currentl selected tile."));
				tile_LayerDepth = EditorGUILayout.IntField (new GUIContent ("Layer Depth: ", "The depth the sprite will be placed at. Default depth is 0."), tile_LayerDepth, EditorStyles.numberField);
				tile_Stackable = EditorGUILayout.Toggle (new GUIContent ("Stack this Object?", "Check this button if you want to place your currently selected tile ontop of another tile. This is useful if you would like to build a miscellaneous item ontop of an environmental building block. (Limited to two tiles stacked up)"), tile_Stackable, EditorStyles.toggle);
				GUILayout.Space (5);
				...
				scrollPos = GUI.BeginScrollView (new Rect (0, 0, TileArea.width, TileArea.height), scrollPos, new Rect (0, 0, 400, (Mathf.Ceil (temporaryKeywordSpriteList.Count / 
		                                                                                                                                (maxTilesPerRowEditorWindow != 0 ? maxTilesPerRowEditorWindow : maxTilesPerRowEditorWindow + 1)) + 1) * (tileWidthAndOffset)));
		...
		private void GridUpdate (SceneView sceneView)
		{
				...
				mousePositionGUI = HandleUtility.GUIPointToWorldRay (Event.current.mousePosition).origin;
				...
				if ((mousePositionGUI.x > 0 && mousePositionGUI.x < size_x * grid_WidthHeight.x) && (mousePositionGUI.y > 0 && mousePositionGUI.y < size_y * grid_WidthHeight.y)) {
						...
						bool addRectOffset = grid_WidthHeight.x % 2 > 0;
            
						int hoverTile_x = Mathf.FloorToInt (aligned.x);
						int hoverTile_y = Mathf.FloorToInt (aligned.y);
            
						verts [0] = new Vector3 (hoverTile_x - grid_WidthHeight.x / 2f, hoverTile_y + grid_WidthHeight.y / 2f, 0) + (addRectOffset ? handleRectOffset : handleRectOffsetZero);
						verts [1] = new Vector3 (hoverTile_x + grid_WidthHeight.x / 2f, hoverTile_y + grid_WidthHeight.y / 2f, 0) + (addRectOffset ? handleRectOffset : handleRectOffsetZero);
						verts [2] = new Vector3 (hoverTile_x + grid_WidthHeight.x / 2f, hoverTile_y - grid_WidthHeight.y / 2f, 0) + (addRectOffset ? handleRectOffset : handleRectOffsetZero);
						verts [3] = new Vector3 (hoverTile_x - grid_WidthHeight.x / 2f, hoverTile_y - grid_WidthHeight.y / 2f, 0) + (addRectOffset ? handleRectOffset : handleRectOffsetZero);
